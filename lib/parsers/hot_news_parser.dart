...
class HotNewsParser extends BaseParser {
  ...
  static const Map<String, String?> _categoryUrls = {
    ...
    'https://hotnews.ro/c/dosare-juridice': 'Politică internă',
    'https://hotnews.ro/c/revista-presei': 'World',
  };
  ...
  Future<List<Article>> _parseCategoryPage(
    ...
    for (final article in articleNodes) {
      try {
        ...
        var description = article.querySelector('.entry-excerpt')?.text.trim() ?? '';

        // Check for sponsored content
        final sponsoredText = article.querySelector('.supratitlu')?.text.trim();
        if (sponsoredText != null && sponsoredText.isNotEmpty) {
          description = '$sponsoredText${description.isEmpty ? '' : ' - $description'}';
        }

        // Extract image
        String? imageUrl;
        final imgElement = article.querySelector('.post-thumbnail img');
        if (imgElement != null) {
          imageUrl = imgElement.attributes['src'] ?? imgElement.attributes['srcset']?.split(' ').first;
        }
  ...
  DateTime _parseDate(Element article) {
    ...
    // Parse Romanian date format (e.g., "23 ian.")
  ...
  DateTime _parseRomanianDate(String dateText) {
    ...
    // Match pattern: "23 ian." or "23 ian"
    final match = RegExp(r'(\d{1,2})\s+([a-z]+)\.?').firstMatch(dateText.toLowerCase());
